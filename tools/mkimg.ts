...
const { values, positionals } = parseArgs({
  ...
  options: {
    ...
    stipple: { type: "boolean", default: false },
  },
  allowPositionals: true,
});

if (positionals.length < 2) {
  console.error("Usage: bun tools/mkimg.ts [--fs] [--stipple] in.png OUT.IMG");
  process.exit(1);
}
...
/* Stipple mode: Mode 13h has no real alpha. Mid-alpha pixels get dithered
 * onto a checkerboard so half the pixels become transparent (idx 0) and
 * the other half opaque, producing a visible "translucent" effect on
 * compiled-sprite / opaque blits.
 *
 *   alpha <  32        : fully transparent (idx 0)
 *   alpha >= 32 & < 224: stipple — write opaque only when ((x + y) & 1) == 0
 *   alpha >= 224       : fully opaque
 *
 * Without --stipple, the original threshold (alpha < 128 = transparent)
 * applies, which is fine for hard-edged sprites. */
function pixelAt(i: number): number {
  const a = rgba[i * 4 + 3];
  if (values.stipple) {
    if (a < 32) return 0;
    if (a < 224) {
    ...
    return nearestColor(rgba[i * 4], rgba[i * 4 + 1], rgba[i * 4 + 2], palette);
  }
  if (a < 128) return 0;
  return nearestColor(rgba[i * 4], rgba[i * 4 + 1], rgba[i * 4 + 2], palette);
}
